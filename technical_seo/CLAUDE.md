...
| `site_auditor.py` | **Built and validated** (v2 enhancements landed) — 15 detectors: per-page (title/meta lengths, h1, canonical, alt text, thin content, schema presence, schema validity vs schema.org required-field map, noindex, redirect chains) plus a cross-page post-pass (duplicate_title, duplicate_meta_description). Persists title/meta/canonical/lang/word_count to `pages` table for cross-page analysis. Folded in canonical_checker's scope. |
