# Channel Profiles

Pick the row that matches the `platform` input. The "Channel focus" column is the
single biggest driver of how the article should read. **Every channel must follow the
Damco Writing Style Guide (`damco-style-guide.md`) and the rules in `writing-rules.md`**
on top of this.

The platform value maps to a folder under `Generated/`:
`SEO Articles`, `Paid Guest Blog`, `Guest Blog`, `Medium`, `LinkedIn`. (For an external
site that is not one of these, the user picks the closest profile.)

## House rules that now apply to ALL channels

- **No FAQ section.** Damco blogs do not use FAQ / "People Also Ask" / "Questions ...
  Ask" sections. Cover those questions inside the body prose instead.
- **Key Takeaways:** include a section with the clear heading **"Key Takeaways"**
  placed immediately after the title (before the intro) on every channel **except
  LinkedIn**. LinkedIn does not use a Key Takeaways box. The section is 3-6 short,
  liftable bullets.
- **Follow the Damco Writing Style Guide** for voice, casing, punctuation, perspective,
  parallelism, accessibility (meaningful link text, never "click here"), and active
  voice. See `damco-style-guide.md`.

---

## SEO Articles  (on-site Damco blog)

- **Channel focus:** Topical depth and search intent come first. Cover the real
  questions a searcher for this keyword has, comprehensively and scannably, *while
  leading with the thesis*. Tip the thesis-vs-coverage balance toward coverage here.
- **Meta title + meta description:** REQUIRED.
- **Key Takeaways:** REQUIRED, heading "Key Takeaways", right after the title.
- **Keyword mechanics:** Strict. Primary keyword in title, ≥1 H2, first two paragraphs,
  and conclusion. Secondary keywords for semantic coverage.
- **Stats:** aim 5+ inline-cited 2025-2026 primary-source stats (hard floor 3).
- **Voice:** Second person (you/your) throughout, consistent. No FAQ.
- **CTA:** Embed brand CTA URL 2-3x naturally.

## Paid Guest Blog  (external publication, paid placement)

- **Channel focus:** Thought leadership and a clear point of view. Open with a narrative
  or a sharp observation, argue a position the host's audience has not read many times,
  and let the prose carry it. Still cover the core subtopics.
- **Tone:** Thought-leadership flow — confident, authoritative, narrative; every claim
  backed by experience, a named example, or a number.
- **Meta title + meta description:** REQUIRED.
- **Key Takeaways:** REQUIRED, heading "Key Takeaways", right after the title.
- **Keyword mechanics:** Natural placement, no stuffing. Primary keyword in title,
  opening, conclusion, and at least one H2.
- **Stats:** ≥3 inline-cited primary-source stats, cited at the point of use. No FAQ.
- **Voice:** First-person / we-voice senior practitioner, or second person — consistent.
- **CTA:** 2-3x, lighter touch; external publications limit self-promotion.

## Guest Blog  /  Medium

- **Channel focus:** Thought-leadership with a personal, opinionated edge. Hook fast,
  take a clear stance, keep paragraphs short, build a narrative one idea to the next,
  and close on a point that invites discussion rather than a tidy summary. Authoritative,
  not casual filler — opinion backed by evidence.
- **Tone:** Thought-leadership flow in first-person / we-voice as a senior Damco
  practitioner (the style guide's SME-blog perspective). Stay in one perspective.
- **Meta title + meta description:** REQUIRED.
- **Key Takeaways:** REQUIRED, heading "Key Takeaways", right after the title.
- **Keyword mechanics:** Natural, light. Primary keyword in title, opening, conclusion,
  and at least one H2.
- **Stats:** ≥3 inline-cited primary-source stats. No FAQ.
- **CTA:** 2-3x, natural.

## LinkedIn  (long-form thought-leadership article)

- **Channel focus:** The most demanding bar. Full thought-leadership flow with SEO/AEO
  quality; only meta title, meta description, and strict keyword *density* are relaxed
  (still place keywords for craft).
- **Tone:** Confident first-person / we-voice senior practitioner. Opinion welcome,
  always backed by experience, a named example, or a number.
- **Required quality elements:**
  - Specific, cited hook in the first two lines (earn the "see more" click).
  - Named real-world examples or clearly-framed representative scenarios with numbers.
  - At least one firsthand "here is what we have seen" observation (EEAT).
  - A comparison table where it earns its place.
  - At least one clean functional infographic (scorecard / process flow / stat band /
    comparison) in brand palette (navy #1F3864, teal #2A9D8F). If image tooling is
    unavailable, render the same content as a comparison table and flag that a designed
    visual should replace it. Provide concise descriptive alt text (accessibility).
- **Key Takeaways:** NOT used on LinkedIn. Do not add one.
- **No FAQ.**
- **Stats:** ≥5 inline-cited stats, all 2024-2026, primary sources only.
- **Listicles ("Top N…"):** see `listicle-rules.md`.
- **CTA:** In the close and 1-2 other natural spots. Close with a POV or a direct
  question to the reader.

---

## Fixed brief defaults (all channels unless overridden)

- **Brand name:** Damco Solutions
- **Target audience:** CIOs, CTOs, IT decision-makers (unless the brief says otherwise)
- **Word count:** 2000-2500. Never exceed 2500. Quality over volume; never pad.
- **Grammar:** US English, Chicago Manual of Style, plus the Damco Writing Style Guide.
- **Known CTA URLs by service** (confirm against the brief / title intent; verify live):
  - Healthcare app development: `https://www.damcogroup.com/healthcare/healthcare-app-development`
  - Microsoft Dynamics 365: `https://www.damcogroup.com/microsoft-dynamics-365-services`
  - AI consulting: `https://www.damcogroup.com/ai-consulting-services`
  - AI integration: `https://www.damcogroup.com/ai-integration-services`
  - AI development: `https://www.damcogroup.com/ai-development-services`
  - Application management (AMS): `https://www.damcogroup.com/application-management-services`
  - Salesforce / general: confirm with the user; the Salesforce paid-blog brief used `https://achieva.ai/`
  - If unsure, ask the user for the exact CTA URL — never guess a slug.
